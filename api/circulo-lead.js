const { registerCirculoLead } = require('./_lib/circuloRegistration');
const {
  isTrustedOrigin,
  isSubmittedTooFast,
  isPlausibleEmail,
...
// Guarda el contacto del Círculo en systeme.io con el tag `acceso-comunidad` ANTES de que
// se abra el checkout de Stripe. Ese orden es el requisito no negociable: acá el ticket
// son $1.000, y un abandono de checkout sin email capturado es la venta más cara que
// puede perder este negocio.
//
...
// con el prefijo "circulo-lead:" para recuperarlo a mano.
//
// Nada de este archivo toca el flujo del evento de septiembre ni el del Bootcamp.
...
module.exports = async (req, res) => {
  ...
  if (!isTrustedOrigin(req)) {
    console.warn('circulo-lead: origen no confiable', req.headers.origin, req.headers.referer);
  ...
  if (empresa) {
    console.warn('circulo-lead: honeypot lleno');
    return res.status(200).json(SILENT_OK);
  }

  if (isSubmittedTooFast(startedAt)) {
    console.warn('circulo-lead: enviado demasiado rápido');
    return res.status(200).json(SILENT_OK);
  }
  ...
  try {
    ...
    // a pagar $1.000. Queda en el log para recuperar el registro a mano.
    console.error('circulo-lead: fallo el registro en systeme.io para', cleanEmail, err);
  }
